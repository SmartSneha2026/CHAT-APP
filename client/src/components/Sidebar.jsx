        ...
        {filteredUsers?.length>0 && filteredUsers.map((user)=> {
          return (<button 
            ...
            className={`w-full p-3 flex items-center gap-3 transition-colors rounded-md ${selectedUser?._id === user._id
              ? "bg-gray-200 ring-gray-200" : "hover:bg-gray-200"
            }`}
          >
            {/* AVATAR */}
            <div className='relative mx-auto lg:mx-0'>
              <img src={user?.avatar?.url || "/avatar-holder.avif"}
                alt={"/avatar-holder.avif"}
                className='w-12 h-12 object-cover rounded-full'
              />
              {onlineUsers.includes(user._id) && (<span className='absolute bottom-0 right-0 w-3 h-3 bg-green-50 rounded-full ring-2 ring-white'/>)}
            </div>
            {/* USER INFO */}
